...
const itemCategory = {
     ...
     * @param {number} isFuzzy 0/精準 || 1/模糊
    ...
    async selectColumns(columns = "" ,wheres = "" ,logic = "AND"  ,isFuzzy = 0 , orders = "" , pageSize = 1 ,page = 1 ){
        ...
        //order 排序判斷
        //TODO 不同排序方式? contents ASC ,create_time DESC
        if(orders){
     ...
     * @returns
     */
    async updateItemCategory(targets = {} ,item_category_id = ""){
        if(isEmptyObject(targets) || !item_category_id) throw new Error ("未帶入參數");
    ...
    /**
     * 根據id進行軟刪除
     * @param {string} item_category_id PK
     * @param {string} update_uid
     * @returns
     */
    async softDeleteItemCategory(item_category_id = "" ,update_uid = ""){
        const sql = `UPDATE ?? SET ? WHERE ?`;
        const targets = {
            status:-1,
            update_uid
